...
def generate_changelog(old_path, new_path, config_path, changelog_file):
    ...
    MODPACKS_FORMAT, old_json, old_config_hash, old_overrides = get_json(
        None, old_path
    )  # None means that the format is not yet known, so it will be whatever detected by the function
    MODPACKS_FORMAT, new_json, new_config_hash, new_overrides = get_json(
        MODPACKS_FORMAT, new_path
    )  # The format is now known, so the function checks it is the same as the old one
