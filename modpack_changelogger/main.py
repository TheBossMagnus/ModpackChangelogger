...
def generate_changelog(old_path, new_path, config_path, changelog_file):
    ...
    MODPACKS_FORMAT, old_json, old_config_hash, old_script_hash, old_overrides = get_json(None, old_path)  # None because that the format is not yet known, it will be whatever detected by the function
    MODPACKS_FORMAT, new_json, new_config_hash, new_script_hash,  new_overrides = get_json(MODPACKS_FORMAT, new_path)  # The format of the first pack is now known, so the function checks it is the same as the second one
    ...
    # Compare the packs
    added, removed, updated = compare_packs(MODPACKS_FORMAT, old_ids, new_ids, old_info, new_info, old_config_hash, new_config_hash, old_script_hash, new_script_hash, config)
