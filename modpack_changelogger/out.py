...
def write_to_file(filename, text):
    if os.path.isdir(filename):
        print("ERROR: The path provided (%s) is a directory, not a file.", filename)
        sys.exit(1)
    try:
        with open(filename, "w", encoding="utf-8") as f:
            f.write(text)
    except FileNotFoundError:
        print("ERROR: The folder selected for the changelog file (%s) doesn't exist.", filename)
        sys.exit(1)
    except PermissionError:
        print("ERROR: You don't have access to the file selected for the changelog (%s). Try running as administrator", filename)
        sys.exit(1)
